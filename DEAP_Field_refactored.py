...
class Genetic:
    ...
    def bounds_fn(self, ind: creator.Individual):
    ...
    def decode_all_x(self, individual: creator.Individual):
        len_chromosome_one_var = self.len_of_turn
        bound_index = 0
        x = []
    ...
    def objective_fxn(self, individual: creator.Individual):
    ...
    def mutate(self, ind: creator.Individual, Indpb: float):
        p = random.random()
        if p <= 0.5:
    ...
    def check_feasibility(self, ind: creator.Individual):
        if self.length(ind) > 100:
            return False
        else:
            return True

    def length(self, ind: creator.Individual):
