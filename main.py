...
if GA.figure == 'Circular':
    calc_radius = max(radii_array) * GA.spacing
    ...
    coords_array = Bz.Radii_in_coords(R=flat_radii_array, coords_max=GA.coords)

    length = Resistance.length_piecewise_linear_coils(coils=coords_array)
...
st.write(f'These are the proportion coefficients (radii in cm):')
radii_array_round = []
for array in radii_array:
    radii_array_round.append(list(map(lambda x: round(100*x, 2), array)))
st.write(radii_array_round)
st.write(f'This is the resistance of your coil (Ohm):')
st.write(resistance)
st.write(f'This is the total length of your coil (m):')
st.write(sum(length))
st.pyplot(coil_pic, dpi=300)
st.pyplot(field_pic_2d, dpi=300)
st.pyplot(field_pic_3d, dpi=300)
